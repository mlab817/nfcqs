...
use App\Models\SrcBaselineData;
...
class DashboardController extends Controller
{
    ...
    public function result(Request $request)
    {
        ...
        $cropId = $request->input('key',1);
        $displayTable = $request->input('table','area');
        ...
        // crop details
        $crop = Crop::with(['province','commodity'])->find($cropId ?? 1);
//        $crop = DB::table('crop')
//            ->select(
...
//
//        // get timeline
        $years = Population::where('year', '>=', $startYear)
            ->where('year', '<=', $endYear)
...
//
//        // get baseline result
        $data = SrcBaselineData::with(['src_baseline'])
            ->where('year', '>=', $startYear)
            ->where('year', '<=', $endYear)
            ->whereHas('src_baseline', function ($q) use ($crop) {
                $q->where('src_province_id', $crop->province_id)
...
//            'chart' => $chart,
    ...
    private function getCommonYear($cropId, $crop)
    {
        ...
        // get baseline years
        $baseline = SrcBaselineData::with('src_baseline')->where('src_commodity_id', $crop->commodity_id)
