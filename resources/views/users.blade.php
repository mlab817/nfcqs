                    ...
                    @foreach ($users as $key)
                        <tr>
                            ...
                            <td style="text-align:center; font-weight:bold; font-size:11px; text-transform:uppercase">
                                ...
                                @if ($key->is_active == 0)
                                    <a href="{{ route('users.access', ['user' => $key, 'action' => 1]) }}" style="color:orange; margin-right:4px; padding-right:5px; border-right:1px solid #ddd">Activate</a>
                                @else
                                    <a href="{{ route('users.access', ['user' => $key, 'action' => 0]) }}" style="color:orange; margin-right:4px; padding-right:5px; border-right:1px solid #ddd">Deactivate</a>
                                @endif

                                <a href="{{ route('users.destroy', $key) }}" class="delete-record" style="color:red">Delete</a>
                            </td>
                        </tr>
                    @endforeach
                @endif
